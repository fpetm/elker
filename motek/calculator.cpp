...
namespace motek {
  ...
  wear_configs_t generate_wear_variations(const std::vector<std::vector<WearType>>& wear_tuples, bool stattrak) {
    ...
    for (std::vector<WearType> wears : wear_tuples) {
      std::vector<std::array<int, 10>> partitions = create_partitions(wears.size());
      ...
      for (bool stattrak : {false, true}) {
        ...
        for (const size_t id : ids) {
          ...
          for (SkinCondition condition : {BS, WW, FT, MW, FN}) {
			  ...
			  if (stattrak) {
				  switch (condition){
				  case BS: c = BS_ST; break;
				  case WW: c = WW_ST; break;
				  case FT: c = FT_ST; break;
				  case MW: c = MW_ST; break;
				  case FN: c = FN_ST; break;
          case BS_ST: c = BS_ST; break;
				  case WW_ST: c = WW_ST; break;
				  case FT_ST: c = FT_ST; break;
				  case MW_ST: c = MW_ST; break;
				  case FN_ST: c = FN_ST; break;
          default: c = BS_ST; break;
				  }
			  }
            m_PricesCompressed[stattrak][rarity](skin.m_rID * 5 + condition) = skin.m_PricesSell[c];
          }
        }
      }
...
//		ss << tradeup.hash() << ",";
		...
		for (size_t i = 0; i < ids_by_rarity.size()*5; i++) {
			if (mask_big(i) > 0.0f) {
				...
				for (int j = 0; j < amount; j++) {
					...
					if (tradeup.stattrak) {
						switch (condition) {
						case BS: condition = BS_ST; break;
						case WW: condition = WW_ST; break;
						case FT: condition = FT_ST; break;
						case MW: condition = MW_ST; break;
						case FN: condition = FN_ST; break;
						}
					}
	...
	void BruteforceCondition(const Calculator& calculator, std::pair<WearType, wear_config_t> kv, bool stattrak, std::vector<TradeUp>& tradeups) {
...
//      std::cout << ShortStringFromWeaponCondition(k.first) << " : " << k.second << " ; ";
      array1[i] = k.second;
    ...
    return;
      ...
      for (size_t depth = 0; depth < g_MaxDepth; depth++) {
		...
		const std::vector<std::vector<size_t>> combinations = combination(ids_by_rarity[rarity].size(), depth + 1);
	...
	void Calculator::Bruteforce(const std::vector<std::vector<WearType>>& wear_tuples) {
		MT_INFO("Bruteforcing...");

    wear_configs_t wear_configs[2] = { generate_wear_variations(wear_tuples, false),
    ...
    for (size_t stattrak = 0; stattrak <= 1; stattrak++) {
      ...
      for (auto keyvalue : wear_configs[stattrak]) {
        ...
        threads[stattrak].push_back(std::thread(BruteforceCondition, std::cref(*this), keyvalue, stattrak, std::ref(tradeups[stattrak][i])));
        i++;
      }
    }
    ...
    for (size_t stattrak = 0; stattrak <= 1; stattrak++) {
      ...
      for (auto keyvalue : wear_configs[stattrak]) {
        tradeups[stattrak].push_back(std::vector<TradeUp>());
        BruteforceCondition(std::cref(*this), keyvalue, stattrak, std::ref(tradeups[stattrak][i]));
        i++;
      }
		}
#endif
