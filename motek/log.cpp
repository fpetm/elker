...
namespace motek {
	...
	void Log::Init() {
		...
		sinks.emplace_back(std::make_shared<spdlog::sinks::basic_file_sink_mt>("motek.log"));

		sinks[0]->set_pattern("%^[%T] %n: %v%$");
		sinks[1]->set_pattern("[%T] [%l] %n: %v");

		s_MTLogger = std::make_shared<spdlog::logger>("motek", begin(sinks), end(sinks));
		spdlog::register_logger(s_MTLogger);
		s_MTLogger->set_level(spdlog::level::trace);
		s_MTLogger->flush_on(spdlog::level::trace);

		s_EKLogger = std::make_shared<spdlog::logger>("elker", begin(sinks), end(sinks));
