...
std::string StringFromWeaponCondition(SkinCondition condition) {
  switch (condition) {
  case SkinCondition::BS:
    return "Battle-Scarred";
  case SkinCondition::WW:
    return "Well-Worn";
  case SkinCondition::FT:
    return "Field-Tested";
  case SkinCondition::MW:
    return "Minimal Wear";
  case SkinCondition::FN:
    return "Factory New";
  case SkinCondition::BS_ST:
    return "StatTrak Battle-scarred";
  case SkinCondition::WW_ST:
    return "StatTrak Well-worn";
  case SkinCondition::FT_ST:
    return "StatTrak Field-tested";
  case SkinCondition::MW_ST:
    return "StatTrak Minimal Wear";
  case SkinCondition::FN_ST:
    return "StatTrak Factory New";
  case SkinCondition::Max:
    return "MAX";
  default: {
...
SkinCondition ConditionFromFloat(WearType wear, bool st) {
  if (st) {
    if (wear >= g_WearRangeMin && wear <= g_WearRange1)
      ...
      return SkinCondition::MW_ST;
    else if (wear > g_WearRange2 && wear <= g_WearRange3)
      return SkinCondition::FT_ST;
    else if (wear > g_WearRange3 && wear <= g_WearRange4)
      return SkinCondition::WW_ST;
    else if (wear > g_WearRange4 && wear <= g_WearRangeMax)
      return SkinCondition::BS_ST;
  } else {
    if (wear >= g_WearRangeMin && wear <= g_WearRange1)
      return SkinCondition::FN;
    else if (wear > g_WearRange1 && wear <= g_WearRange2)
      return SkinCondition::MW;
    else if (wear > g_WearRange2 && wear <= g_WearRange3)
      return SkinCondition::FT;
    else if (wear > g_WearRange3 && wear <= g_WearRange4)
      return SkinCondition::WW;
    else if (wear > g_WearRange4 && wear <= g_WearRangeMax)
      return SkinCondition::BS;
  }
...
SkinDB::SkinDB(std::string skinpath) {
  ...
  for (auto &row : data) {
    ...
    if (row[2] == "Consumer Grade")
      rarity = SkinRarity::Consumer;
    else if (row[2] == "Industrial Grade")
      rarity = SkinRarity::Industrial;
    else if (row[2] == "Mil-Spec Grade")
      rarity = SkinRarity::MilSpec;
    else if (row[2] == "Restricted")
      rarity = SkinRarity::Restricted;
    else if (row[2] == "Classified")
      rarity = SkinRarity::Classified;
    else if (row[2] == "Covert")
      rarity = SkinRarity::Covert;
    else if (row[2] == "Contraband")
      continue;
    else
      MT_ERROR("Unknown rarity (skin: {}): {}", row[0], row[2]);
    ...
    for (SkinCondition condition :
      ...
      for (Skin &skin : collection) {
        if (skin.m_Rarity >= collection.m_HighestRarity ||
            skin.m_PricesSell[condition] <= 0.03 ||
            skin.m_PricesBuy[condition] <= 0.03) {
